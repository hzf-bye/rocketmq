...
public class SendMessageTraceHookImpl implements SendMessageHook {
    ...
    public void sendMessageBefore(SendMessageContext context) {
        ...
        //如果消息topic是轨迹消息topic，直接返回
        if (context == null || context.getMessage().getTopic().startsWith(((AsyncTraceDispatcher) localDispatcher).getTraceTopicName())) {
            return;
        }
        //build the context content of TuxeTraceContext
        //在消息发送上下文中，设置用来跟踪消息轨迹的上下环境，里面主要包含一个TraceBean集合、追踪类型(TraceType.Pub)与生产者所属的组。
    ...
    public void sendMessageAfter(SendMessageContext context) {
        ...
        /*
         * 设置costTime(耗时)、success(是否发送成功)、regionId(发送到broker所在的分区)、msgId(消息ID，全局唯一)、
         * offsetMsgId(消息物理偏移量，如果是批量消息，则是最后一条消息的物理偏移量)、storeTime，
         * 这里使用的是(客户端发送时间 + 二分之一的耗时)来表示消息的存储时间，这里是一个估值。
         */
